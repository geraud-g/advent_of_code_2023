...
def part_one(input_file: str) -> int:
    ...
    possible_games_ids = [
        game.id for game in games if has_game_enough_cubes(game, max_cubes)
    ]
...
def get_input(input_file: str) -> list[Game]:
    ...
    with open(input_file) as f:
        for line in f.readlines():
            ...
            games.append(
                Game(id=game_id, max_colors_nbr=Cubes(red=red, blue=blue, green=green))
            )
    return games
...
def get_game_power(game: Game) -> int:
    return (
        game.max_colors_nbr.red * game.max_colors_nbr.green * game.max_colors_nbr.blue
    )
